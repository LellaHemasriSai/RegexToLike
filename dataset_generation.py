...
def main():
    # Load the regex patterns from 'regex_patterns.csv'
    df = pd.read_csv('regex_like_dataset.csv')

    # Specify the range of indices you want to process
    start_index = 1750  # Change this to your desired start index
    end_index = 1984  # Change this to your desired end index

    # Slice the dataframe to get the specific range
    sample_df = df.iloc[start_index:end_index].reset_index(drop=True)
    ...
    for index, row in tqdm(sample_df.iterrows(), total=sample_df.shape[0]):
        pattern = row['regex']
        # Generate the SQL query using the provided code
    ...
    outputfile='regex_sql_pairs'+str(start_index)+'_'+str(end_index)+'.csv'
    output_df.to_csv(outputfile, index=False)

    # Prepare data for fine-tuning LLaMA 2
    llama_data = []
    for item in data:
        instruction = "Convert the following regex pattern to an Oracle SQL query using LIKE where possible."
        input_text = item['regex']
        output_text = item['sql_query']
        llama_data.append({
    ...
    # Save to JSON file
    jsonfile='llama_finetune_data'+str(start_index)+'_'+str(end_index)+'.json'
    with open(jsonfile, 'w') as f:
        json.dump(llama_data, f, indent=2)

    print("Datasets have been generated and saved.")
